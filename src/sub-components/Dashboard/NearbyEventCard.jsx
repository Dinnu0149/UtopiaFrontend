...
import { getFirstCharacters, formatCurrency } from "../../utils/HelperFunc";
import EventModelPopUp from "../Event/EventModelPopUp";
import FavoriteButton from "../../components/Button/FavoriteButton";

function NearbyEventCard({ event, handleRemovePop }) {
    ...
    <Card className={`px-3 shadow-sm ${styles["event-card"]}`}>
      <Row>
        <Col lg={12} xs={12} md={12} sm={12} className="m-0 p-0">
          ...
          <Row className={styles["nearbyEvent"]}>
            <Col className={`${styles["nearbyEventImageContainer"]}`}>
              <div
            ...
            >
              ...
              <Card.Title className={`fw-bold mb-0 ${styles["eventTitle"]} `}>
                {getFirstCharacters(event.name, 15)}...
              </Card.Title>
